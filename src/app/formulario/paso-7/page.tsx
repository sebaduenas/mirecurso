...
import { DOCUMENTOS_REQUERIDOS } from '@/data/datos-fijos';
...
export default function Paso7Page() {
  ...
  useEffect(() => {
    if (!isStepAccessible(7)) {
      router.replace('/formulario/paso-6');
      return;
    }
    setCurrentStep(7);
  ...
  const documentosObligatorios = DOCUMENTOS_REQUERIDOS.filter((d) => d.obligatorio);
                  ...
                  {documentosObligatorios.map((doc) => (
                    <li key={doc.id} className="flex items-start gap-2">
                      ...
                      <span>
                        {doc.nombre}
                        {doc.descripcion && (
                          <span className="text-muted-foreground"> ({doc.descripcion})</span>
                        )}
                      </span>
                    </li>
                  ))}
